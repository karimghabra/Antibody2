...
%p13 = 0.00998996*p14^(n-1)/0.2012^(n-1);       %VmaxD1fast
p14 = 2.85;             %KmD1fast
p14P = 2.85;
p13 = 0.5*0.00998996*p14P^n/(0.2012^(n-1)*p14);       %VmaxD1fast
%p15 = 6.63*10^-4;       %VmaxD1slow
%p15 = 6.63*10^-4*p16^(n-1)/0.6418^(n-1);       %VmaxD1slow
p16 = 95;               %KmD1slow
p16P = 95;               %KmD1slow
p15 = 0.5*6.63*10^-4*p16P^n/(0.6418^(n-1)*p16);       %VmaxD1slow
...
param1 = 1.3;           % boundary parameter
param2 = 0.07;           % rate of T3 increase
...
%elseif (q(5) >= param1*0.012)
%  SR3 = ((p19+param2*q(5))*q(19))*d3; % Brain delay
%endif
SR3 = (p19*q(19))*d3; % Brain delay
...
%NL2 = p13/(p14+q(2));
%n = 2; NL3 = p13*q(2)^n/(p14+q(2)^n);
NL3 = p13*q(2)^n/(p14^n+q(2)^n);
...
%q2dot = p6*q1F-(p3+p12+NL2)*q(2);                    %T4fast2
q2dot = p6*q1F-(p3+p12)*q(2)+NL3;                    %T4fast3
%q3dot = p5*q1F-(p4+p15/(p16+q(3))+p17/(p18+q(3)))*q(3);         %T4slow
%q3dot = p5*q1F-(p4*q(3)+p15*q(3)^n/(p16+q(3)^n)+p17*q(3)/(p18+q(3)));%T4slow2
q3dot = p5*q1F-(p4*q(3)+p15*q(3)^n/(p16^n+q(3)^n)+p17*q(3)/(p18+q(3)));%T4slow2
q4dot = SR3+p20*q(5)+p21*q(6)-(p22+p23)*q4F+p28*q(13)+u4;       %T3pdot
%q5dot = p23*q4F+p13*q(2)/(p14+q(2))-(p20+p29)*q(5);             %T3fast
%q5dot = p23*q4F+NL2*q(2)-(p20+p29)*q(5);             %T3fast2
q5dot = p23*q4F+NL3-(p20+p29)*q(5);             %T3fast3
%q6dot = p22*q4F+p15*q(3)/(p16+q(3))+p17*q(3)/(p18+q(3))-p21*q(6);%T3slow
%q6dot = p22*q4F+p15*q(3)^n/(p16+q(3)^n)+p17*q(3)/(p18+q(3))-p21*q(6);%T3slow2
q6dot = p22*q4F+p15*q(3)^n/(p16^n+q(3)^n)+p17*q(3)/(p18+q(3))-p21*q(6);%T3slow2
...
% Conversion factors. Molecular weight over deciliters
T4conv  = 777/30;
T3conv  = 651/p47;
TSHconv = 5.6/p48;
